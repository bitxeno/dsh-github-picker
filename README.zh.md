...
列表每页加载 12 条，滚动到底部自动获取下一页，直到取完所有结果（无上限）。
...
插入格式（`insertFormat`）是**设置 → GitHub 引用**里的持久化选项，无需重启 Host 即可生效；结果列表没有数量上限，始终滚动加载到全部结果。
...
- 弹窗按页加载最近列表（每页 12 条），滚动到底部自动获取下一页，直到取完所有结果（无上限）；结果按会话缓存 30 秒（TTL 内重开即时显示、超时自动重取），之后本地过滤，输入不会堆积请求。
